...
def C5N_PMOS_GEN(cv, w=1.8, l=0.6, m=1, leftSub=1, rightSub=0) :
	lib = cv.lib()
	...
	r = Rect(int((-height/2) - left_ext), int(-width/2), int((height/2) + right_ext), int(width/2))
	active = cv.dbCreateRect(r, layer);

	# # Create nwell
	nwell = tech.getLayerNum("nwell", "drawing")
	r = Rect(int((-height/2) - left_ext - nwell_ovlp_active), 
			int(-width/2 - nwell_ovlp_active), 
			int((height/2) + right_ext + nwell_ovlp_active), 
			int(width/2 + nwell_ovlp_active))
	cv.dbCreateRect(r, nwell);
	...
	r = Rect(int(-height/2 - left_ext_sel), 
		 int(-width/2 - select_ovlp_active), 
		 int(height/2 + right_ext_sel),
		 int(width/2 + select_ovlp_active))
	...
	if leftSub :
		r = Rect(int(-height/2 - sub_sel_height), 
			 int(-width/2 - select_ovlp_active), 
			 int(-height/2),
			 int(width/2 + select_ovlp_active))
	...
	for i in range(fingers) :
		p = Rect(int(xoffset), int(-width/2 - poly_ovlp_active), int(xoffset + length), int(width/2 + poly_ovlp_active))
	...
	# Create S/D contacts
	contact = tech.getLayerNum("contact", "drawing")
	numCuts = int(width / (cut_width + cut_space))
	...
	xoffsetL = int(-height/2 - cut_width - active_ovlp_cut - int(0.6*dbu))
	xoffsetR = int(height/2 + active_ovlp_cut + int(0.6*dbu))
	if (leftSub) :
		yoffset = int(active_ovlp_cut + yfudge - width/2)
		for j in range(numCuts) :
			cut = Rect(0, 0, cut_width, cut_width)
	...
	for i in range(fingers + 1) :
		met = Rect(0, int(-width/2), int(cut_width + 2*metal_ovlp_cut), int(width/2))
		...
		xoffset = xoffset + length + 2*poly_to_cut + cut_width
	if leftSub :
		met = Rect(int(-height/2 - active_ovlp_cut - cut_width - metal_ovlp_cut - int(0.6*dbu)),
					int(-width/2),
					int(-height/2 - active_ovlp_cut + metal_ovlp_cut - int(0.6*dbu)), 
					int(width/2))
	...
	if rightSub :
		met = Rect(int(height/2 + active_ovlp_cut - metal_ovlp_cut + int(0.6*dbu)),
					int(-width/2),
					int(height/2 + active_ovlp_cut + cut_width + metal_ovlp_cut + int(0.6*dbu)), 
					int(width/2))
