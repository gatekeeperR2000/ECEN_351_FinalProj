...
def C5N_NMOS_GEN(cv, w=1.8, l=0.6, m=1, leftSub=1, rightSub=0) :
	...
	sub_sel_height = int(2*active_ovlp_cut + cut_width + select_ovlp_active + int(0.6*dbu))
	if leftSub :
	...
	for i in range(fingers) :
		p = Rect(xoffset, int(-width/2 -poly_ovlp_active), int(xoffset + length), int(width/2 + poly_ovlp_active))
