...
def nch(cv, w=1.1e-6, l=0.13e-6, nf=1, polyCnt=False, leftCnt=True,
rightCnt=True) :
	lib = cv.lib()
	tech = lib.tech()
	dbu = float(cv.dbu())
	...
	if cut_space <= 0 :
		...
		raise ValueError
	pwell_ovlp_active = tech.getLayerEnc('nwell', 'drawing', 'od', 'drawing')
	...
	if poly_ovlp_cut <= 0 :
		...
		raise ValueError
	nplus_ovlp_active = tech.getLayerEnc('nimp', 'drawing', 'od', 'drawing')
	...
	r = Rect(int(-height/2 - nplus_ovlp_active), int(-width/2 - nplus_ovlp_active), 
		     int(height/2 + nplus_ovlp_active),  int(width/2 + nplus_ovlp_active))
