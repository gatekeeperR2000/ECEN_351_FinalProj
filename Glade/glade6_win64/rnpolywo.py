...
def rnpolywo(cv, w=0.66e-6, r=5000.0, numsegs=1) :
	lib = cv.lib()
	tech = lib.tech()
	dbu = float(cv.dbu())
	...
	if rpo_ovlp_poly <= 0 :
		...
		raise ValueError
	nplus_ovlp_poly = tech.getLayerEnc('nimp', 'drawing', 'polyg', 'drawing')
	if nplus_ovlp_poly <= 0 :
	...
	nplus_width = width + nplus_ovlp_poly * 2
	ypts[0] = int(ypts[0] - (width/2 + nplus_ovlp_poly))
	if numsegs % 2 :
		ypts[numPoints-1] = int(ypts[numPoints-1] + width/2 + nplus_ovlp_poly)
	else :
		ypts[numPoints-1] = int(ypts[numPoints-1] - (width/2 + nplus_ovlp_poly))
	nplus = cv.dbCreatePath(xpts, ypts, numPoints, layer, nplus_width, 0, 0, 0)
	# Restore ptlist
	for i in range(0, numPoints) :
		xpts[i] = xsavepts[i]
		ypts[i] = ysavepts[i]
